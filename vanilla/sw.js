...
self.addEventListener("fetch", (event) => {
  ...
  // Strategy 1: Stale-While-Revalidate (For OpenMensa API)
    ...
    event.respondWith(
      caches.open(API_CACHE_NAME).then(async (cache) => {
        ...
        const networkFetch = fetch(event.request)
          ...
          .catch(() => cachedResponse); // Fall back to cache on offline error

        // Instantly return cached response if available; otherwise wait for network
